...
func (m *postgresMacroEngine) evaluateMacro(timeRange backend.TimeRange, query *backend.DataQuery, name string, args []string) (string, error) {
	...
	case "__unixEpochMilliFilter":
		...
		return fmt.Sprintf("%s >= %d AND %s <= %d", args[0], timeRange.From.UTC().UnixMilli(), args[0], timeRange.To.UTC().UnixMilli()), nil
	...
	case "__unixEpochMilliFrom":
		return fmt.Sprintf("%d", timeRange.From.UTC().UnixMilli()), nil
	case "__unixEpochMilliTo":
		return fmt.Sprintf("%d", timeRange.To.UTC().UnixMilli()), nil
	...
	case "__unixEpochMilliGroup":
		...
		return fmt.Sprintf("floor((%s)/%v)*%v", args[0], interval.Milliseconds(), interval.Milliseconds()), nil
